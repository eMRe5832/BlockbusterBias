...
Initial Step
Get predictions for ten algorithms on five datasets via Surprise library in Python (see Predict.py) and save predicted scores as "DatasetName-AlgorithmName.txt" in their corresponding folder in the Predictions file. Note that in the Datasets folder, we store the datasets as .mat file. The ML-100K and ML-1M datasets are stored as user-item matrix form, while other datasets are stored as userID|itemID|Rating as nx3 format where n is the total number of provided ratings.
